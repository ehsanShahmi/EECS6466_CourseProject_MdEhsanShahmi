...
class TestTaskFunc(unittest.TestCase):
    ...
    def test_single_tuple_input(self):
        """Test with a single tuple containing small numbers"""
        result = task_func((('2', '3'),))
        self.assertTrue(all(0 <= number <= 100 for number in result.keys()))

    def test_multiple_tuples_input(self):
        """Test with multiple tuples of small numbers"""
        result = task_func((('1', '2'), ('3', '4')))
        self.assertTrue(all(0 <= number <= 100 for number in result.keys()))

    def test_large_numbers_input(self):
        """Test with larger numbers in tuples"""
        result = task_func((('10', '20'), ('30', '40'), ('50', '60')))
        self.assertTrue(all(0 <= number <= 100 for number in result.keys()))

    def test_range_limit(self):
        """Test with a specific range limit"""
        result = task_func((('1', '1'),), RANGE=5)
        self.assertTrue(all(0 <= number <= 5 for number in result.keys()))
