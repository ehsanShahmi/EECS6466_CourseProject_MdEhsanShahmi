...
class TestTaskFunction(unittest.TestCase):
    ...
    def test_default_sales_data_shape(self):
        report = task_func()
        self.assertEqual(report.shape, (60, 3))  # 12 months * 5 categories = 60 rows

    def test_custom_categories(self):
        custom_categories = ['Toys', 'Games']
        report = task_func(categories=custom_categories)
        self.assertTrue(all(cat in custom_categories for cat in report['Category'].unique()))
        
    def test_custom_months(self):
        custom_months = ['January', 'February']
        report = task_func(months=custom_months)
        self.assertTrue(all(month in custom_months for month in report['Month'].unique()))
        
    def test_invalid_categories(self):
        with self.assertRaises(ValueError):
            task_func(categories=None)
