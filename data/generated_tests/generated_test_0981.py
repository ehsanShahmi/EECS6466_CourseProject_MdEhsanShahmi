...
class TestTaskFunc(unittest.TestCase):
    def test_valid_input(self):
        df, ax = task_func('2020-01-01', '2020-01-10', 2)
        self.assertEqual(df.shape[0], 10)  # 10 days
        self.assertEqual(df.shape[1], 2)   # 2 series

    def test_dates_order(self):
        with self.assertRaises(ValueError):
            task_func('2020-01-10', '2020-01-01', 2)

    def test_single_series(self):
        df, ax = task_func('2020-01-01', '2020-01-10', 1)
        self.assertTrue('series_1' in df.columns)
        self.assertEqual(df.shape[1], 1)  # 1 series only

    def test_no_series(self):
        with self.assertRaises(ValueError):
            task_func('2020-01-01', '2020-01-10', 0)

    def test_random_seed(self):
        df1, _ = task_func('2020-01-01', '2020-01-10', 3, seed=42)
        df2, _ = task_func('2020-01-01', '2020-01-10', 3, seed=42)
        pd.testing.assert_frame_equal(df1, df2)  # DataFrames should be identical with the same seed
