...
class TestTaskFunc(unittest.TestCase):
    ...
    def test_dataframe_shape(self):
        df, stats = task_func(10)
        self.assertEqual(df.shape, (10, 6))

    def test_default_column_names(self):
        df, stats = task_func(10)
        self.assertListEqual(list(df.columns), ['A', 'B', 'C', 'D', 'E', 'F'])

    def test_mean_and_median_calculation(self):
        df, stats = task_func(10)
        for col in df.columns:
            self.assertAlmostEqual(stats[col]['mean'], df[col].mean(), places=5)
            self.assertAlmostEqual(stats[col]['median'], df[col].median(), places=5)

    def test_invalid_rows_value(self):
    ...
    def test_custom_columns(self):
        ...
        df, stats = task_func(10, columns=custom_columns)
        self.assertListEqual(list(df.columns), custom_columns)
